...
public:
    ...
    size_t MulticastBinary(const void* buffer, size_t size) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_BINARY, buffer, size); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }
    size_t MulticastBinary(std::string_view text) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_BINARY, text.data(), text.size()); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }

    // WebSocket multicast ping methods
    size_t MulticastPing(const void* buffer, size_t size) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_PING, buffer, size); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }
    size_t MulticastPing(std::string_view text) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_PING, text.data(), text.size()); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }

    // WebSocket multicast pong methods
    size_t MulticastPong(const void* buffer, size_t size) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_PONG, buffer, size); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }
    size_t MulticastPong(std::string_view text) { std::scoped_lock locker(_ws_send_lock); PrepareSendFrame(WS_FIN | WS_PONG, text.data(), text.size()); return HTTPSServer::Multicast(_ws_send_buffer.data(), _ws_send_buffer.size()); }
